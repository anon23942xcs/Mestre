...
def _escopo(campanha_id: str) -> str:
    return f"campanha_{campanha_id}"
...
async def listar_fichas(campanha_id: str):
    ...
    return ficha_repositorio.listar(_escopo(campanha_id))


@router.get("/{ficha_id}", response_model=FichaMundo)
async def obter_ficha(campanha_id: str, ficha_id: str):
    _exigir_campanha(campanha_id)
    ficha = ficha_repositorio.carregar(_escopo(campanha_id), ficha_id)
    if not ficha:
...
async def criar_ficha(campanha_id: str, dados: FichaMundoRequest):
    ...
    ficha_repositorio.salvar(_escopo(campanha_id), ficha)
    return ficha
...
async def apagar_ficha(campanha_id: str, ficha_id: str):
    _exigir_campanha(campanha_id)
    if not ficha_repositorio.deletar(_escopo(campanha_id), ficha_id):
        raise HTTPException(status_code=404, detail="Ficha não encontrada")
